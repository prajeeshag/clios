...
def test_with_build_phase_strict(parameter, type_adapter_mock):
    param_type = Param(
        build_phase_validators=[int, str], core_validation_phase="build", strict=True
    )
    parameter_mock = parameter(param_type)
    parameter_mock._init_build_phase_validator()

    type_adapter_mock.assert_called_once_with(
        Annotated[int, BeforeValidator(int), BeforeValidator(str), Strict()]
    )
    assert parameter_mock._build_phase_validator == "TypeAdapter"


def test_with_build_phase_nostrict(parameter, type_adapter_mock):
    param_type = Param(build_phase_validators=[int, str], core_validation_phase="build")
    parameter_mock = parameter(param_type)
    parameter_mock._init_build_phase_validator()
...
def test_with_execute_phase_nostrict(parameter, type_adapter_mock):
    param_type = Param(
        build_phase_validators=[int, str], core_validation_phase="execute"
    )
    parameter_mock = parameter(param_type)
    parameter_mock._init_build_phase_validator()
...
def test_with_execute_phase_strict(parameter, type_adapter_mock):
    param_type = Param(
        build_phase_validators=[int, str], core_validation_phase="execute", strict=True
    )
    parameter_mock = parameter(param_type)
    parameter_mock._init_build_phase_validator()
