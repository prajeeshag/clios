...
def test_failing(input, mock_arg_parser):
    with pytest.raises(AssertionError) as e:
        OperatorFn.validate(input.fn, param_parser=mock_arg_parser)
...
def test_implicit_invalid(mock_arg_parser):
    ...
    with pytest.raises(AssertionError) as e:
        OperatorFn.validate(dummy, implicit="invalid", param_parser=mock_arg_parser)
...
def test_Parameter_is_input():
    op = OperatorFn.validate(fn, param_parser=mock_arg_parser)
    op1 = OperatorFn.validate(fn, param_parser=mock_arg_parser, implicit="param")
...
def test_Parameter_type_():
    op = OperatorFn.validate(fn, param_parser=mock_arg_parser)
    op4 = OperatorFn.validate(fn4, param_parser=mock_arg_parser)
    op5 = OperatorFn.validate(fn5, param_parser=mock_arg_parser)
...
def test_iter_positional():
    op = OperatorFn.validate(fn, param_parser=mock_arg_parser)
    op2 = OperatorFn.validate(fn2, param_parser=mock_arg_parser)
...
def test_ReturnValue_type_():
    op = OperatorFn.validate(fn, param_parser=mock_arg_parser)
    op1 = OperatorFn.validate(fn3, param_parser=mock_arg_parser)
